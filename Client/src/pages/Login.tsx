        ...
        <div className='min-h-screen flex flex-col md:flex-row'>
            ...
            <div className='flex-1 flex flex-col items-start justify-between p-6 md:p-10 lg:pl-40'>
                ...
                <div>
                    ...
                    <p className='text-xl md:text-2xl text-indigo-900 max-w-72 md:max-w-md'>Join LoopIn-Connecting You to a Thriving Global Community.</p>
                </div>
            ...
            <div className='flex-1 flex items-center justify-center p-6 sm:p-10 lg:pr-40'>
                <div className='w-full max-w-md bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-8'>
                    <h2 className='text-2xl font-bold text-gray-800 mb-6 text-center'>{isLogin ? 'Welcome Back' : 'Create Account'}</h2>
                    <form onSubmit={handleSubmit} className='space-y-4'>
                        {!isLogin && (
                            <>
                                <input type='text' value={fullName} onChange={(e) => setFullName(e.target.value)} required className='w-full px-4 py-2 border rounded-lg' placeholder='Full Name' />
                                <input type='text' value={username} onChange={(e) => setUsername(e.target.value)} className='w-full px-4 py-2 border rounded-lg' placeholder='Username (optional)' />
                            </>
                        )}
                    ...
                    <button onClick={() => (window.location.href = `${API_BASE_URL}/api/auth/google`)} className='w-full mt-4 border py-2 rounded-lg'>
